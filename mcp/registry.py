...
mcp_server = FastMCP("RoboResearch Registry")
...
def _read_metadata(run_id: str) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(f"No experiment found: {run_id}")
...
def save_checkpoint(
    ...
    """Save a model checkpoint and metadata to the registry.

    Args:
        run_id: Unique identifier for this experiment run.
        model_path: Path to the saved SB3 model file.
        config: Training configuration (algorithm, env_name, hyperparams).
        metrics: Evaluation metrics (success_rate, mean_reward).
    ...
    row = f"{run_id}\t{timestamp}\t{algorithm}\t{env_name}\t{success_rate}\t{mean_reward}\t{notes}"
...
def list_experiments(
    ...
    Args:
        filter_by: Optional filters (env_name, algorithm, min_success_rate).
        sort_by: Sort key (timestamp, success_rate, reward).
    ...
    if filter_by:
        if "env_name" in filter_by:
            target = filter_by["env_name"]
            experiments = [e for e in experiments if e.get("config", {}).get("env_name") == target]
        if "algorithm" in filter_by:
            target = filter_by["algorithm"]
            experiments = [e for e in experiments if e.get("config", {}).get("algorithm") == target]
        if "min_success_rate" in filter_by:
            threshold = float(filter_by["min_success_rate"])
            experiments = [
                e for e in experiments
                if float(e.get("metrics", {}).get("success_rate", 0)) >= threshold
            ]
...
def get_best_model(env_name: str, metric: str = "success_rate") -> dict:
    """Find the experiment with the best performance for a given environment.

    Args:
        env_name: Gymnasium environment ID to filter by.
        metric: Metric to maximize (success_rate or mean_reward).
    """
    _ensure_dirs()
    experiments = _all_metadata()
    experiments = [e for e in experiments if e.get("config", {}).get("env_name") == env_name]

    if not experiments:
        raise ValueError(f"No experiments found for {env_name}")
...
def diff_configs(run_id_a: str, run_id_b: str) -> dict:
    """Compare configs between two experiment runs.
